...
## 2025-11-23
- [x] 仔细回顾咱们的防伪任务特点，思考它对全局信息和局部信息的侧重
- [ ] 执行可视化实验
    - [ ] 尝试可视化ViT和当前LocalAttention2D的注意力权重，对比它们在图片上关注的区域
    - [ ] 记录观察结果
- [x] 探索混合注意力策略
    - [x] 尝试构建一个混合模型，例如在模型的前半部分使用`LocalAttention2D`，后半部分使用`AttentionBlock`
    - [x] 或者尝试交替使用这两种注意力模块
- [ ] 研究并考虑实现窗口间交互机制
    - [x] 了解Swin Transformer的`shifted window attention`原理
    - [x] [教程](https://www.bilibili.com/video/BV1GPymBwEu1?t=462.7&p=21)
    - [x] [另外一个教程](https://www.bilibili.com/video/BV13L4y1475U?t=4.6)
...
- [ ] [[zero-shot分类检测适配器方案]]开始做实验并与rine进行对比
- [ ] 赶紧把rine重新看一遍（精读的那种）
- [ ] 下载CASIA v1,v2的数据集（手工篡改数据集）