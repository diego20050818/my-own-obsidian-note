---
tags:
  - code
  - deepfake
  - 对比学习
aliases:
---
[[实验方案与代码实现：R-INE 残差与双模式检测]]
[[深度伪造检测-图像对对比学习研究方案]]
# 🧬 完整模型实现：DeepfakeContrastiveResidual


## 🏗️ 模块化设计说明

为了保证代码的可维护性和可扩展性，我们将模型拆分为以下独立模块：

>1. **`FeatureExtractor`**: 骨干网络（Backbone），负责将图像转换为特征向量。
>2. **`MemoryBank`**: 记忆库，用于存储真实样本的原型特征，支持写入和检索。
>3. **`DifferenceAttention`**: 差异注意力模块，融合特征与差异信息。
>4. **`ResidualHead`**: 残差检测头，专门根据特征距离判断是否异常。
>5. **`DeepfakeContrastiveResidual`**: 主模型，统筹双模式（Pair/Single）逻辑。

    

---

## 💻 PyTorch 代码实现

```python
import torch
import torch.nn as nn
import torch.nn.functional as F
import torchvision.models as models

# ==========================================
# 1. 特征提取器模块 (Feature Extractor)
# ==========================================

-> 重写适配rine
class FeatureExtractor(nn.Module):
    """
    骨干网络模块
    默认使用 ResNet50，可替换为 R-INE 或 EfficientNet。
    去除最后的全连接分类层，只输出特征向量。
    """
    def __init__(self, backbone_name='resnet50', pretrained=True):
        super().__init__()
        if backbone_name == 'resnet50':
            # 加载预训练的 ResNet50
            base_model = models.resnet50(pretrained=pretrained)
            # 移除最后的 fc 层
            self.backbone = nn.Sequential(*list(base_model.children())[:-1])
            self.output_dim = 2048 # ResNet50 的输出维度
        else:
            raise NotImplementedError(f"Backbone {backbone_name} not implemented yet.")
            
    def forward(self, x):
        # Input: [B, 3, 224, 224]
        features = self.backbone(x)
        # Flatten: [B, 2048, 1, 1] -> [B, 2048]
        return torch.flatten(features, 1)

# ==========================================
# 2. 记忆库模块 (Memory Bank)
# ==========================================
这一块的记忆库需要重写 -> 试试使用moca
class MemoryBank(nn.Module):
    """
    记忆库系统
    用于存储真实样本(Real)的特征原型。
    在推理(Single Mode)时，检索最相似的真实特征作为'基准'。
    """
    def __init__(self, feature_dim, bank_size=1024, momentum=0.9):
        super().__init__()
        self.feature_dim = feature_dim
        self.bank_size = bank_size
        self.momentum = momentum
        
        # 注册缓冲区 (不参与梯度更新，但会随模型保存)
        # 初始化为随机正态分布并归一化
        self.register_buffer('features', torch.randn(bank_size, feature_dim))
        self.register_buffer('ptr', torch.zeros(1, dtype=torch.long))
        self.features = F.normalize(self.features, dim=1)
        
    def update(self, keys):
        """
        更新记忆库 (通常在训练阶段处理真实样本时调用)
        keys: [B, Dim]
        """
        with torch.no_grad():
            batch_size = keys.shape[0]
            ptr = int(self.ptr)
            
            # 归一化特征
            keys = F.normalize(keys, dim=1)
            
            # 处理边界情况：如果 batch 超过剩余空间，则截断或循环覆盖
            # 这里简化处理：直接覆盖，若越界则回到头部
            if ptr + batch_size > self.bank_size:
                batch_size = self.bank_size - ptr # 只填满剩余部分
                keys = keys[:batch_size]
            
            # 动量更新或直接替换 (此处采用直接替换用于构建原型)
            self.features[ptr:ptr+batch_size] = keys
            
            # 更新指针
            self.ptr[0] = (ptr + batch_size) % self.bank_size

    def retrieve(self, query):
        """
        检索最相似特征
        query: [B, Dim]
        Return: [B, Dim] (与 query 最相似的 memory feature)
        """
        # 归一化查询
        query = F.normalize(query, dim=1)
        
        # 计算相似度: [B, Dim] @ [Dim, Bank] -> [B, Bank]
        sim = torch.mm(query, self.features.t())
        
        # 找到相似度最高的索引
        best_sim_idx = sim.argmax(dim=1)
        
        # 取出对应的特征
        retrieved_feats = self.features[best_sim_idx] # [B, Dim]
        
        return retrieved_feats

# ==========================================
# 3. 差异注意力与融合 (Difference Attention)
# ==========================================
class DifferenceAttention(nn.Module):
    """
    核心组件：计算交叉注意力
    利用 '差异信息' 来增强 '原始特征'，使模型关注篡改区域。
    """
    def __init__(self, feature_dim, hidden_dim=512):
        super().__init__()
        # 降维处理，减少计算量
        self.query_layer = nn.Linear(feature_dim, hidden_dim)
        self.key_layer = nn.Linear(feature_dim, hidden_dim)
        self.value_layer = nn.Linear(feature_dim, hidden_dim)
        
        # 融合后的恢复层
        self.out_layer = nn.Linear(hidden_dim, feature_dim)
        self.scale = hidden_dim ** -0.5
        
        # 门控机制 (可选)
        self.gate = nn.Sequential(
            nn.Linear(feature_dim * 2, feature_dim),
            nn.Sigmoid()
        )

    def forward(self, original_feat, difference_feat):
        """
        Args:
            original_feat: 原始图像特征 (Query)
            difference_feat: 差异特征 (Key/Value) - 来自 Pair 或 Memory 差分
        """
        # Q, K, V
        Q = self.query_layer(original_feat).unsqueeze(1)    # [B, 1, H]
        K = self.key_layer(difference_feat).unsqueeze(1)    # [B, 1, H]
        V = self.value_layer(difference_feat).unsqueeze(1)  # [B, 1, H]
        
        # Attention Score
        attn = (Q @ K.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1) # [B, 1, 1] 实际上是标量权重
        
        # 加权融合
        context = (attn @ V).squeeze(1) # [B, H]
        context = self.out_layer(context) # [B, Dim]
        
        # 残差连接 + 简单的门控融合
        # 结合原始特征和注意力上下文
        fused = original_feat + context
        return fused

# ==========================================
# 4. 残差检测头 (Residual Head)
# ==========================================
class ResidualHead(nn.Module):
    """
    专门用于判断 '扰动项' 大小的辅助头。
    如果差异很大，说明大概率是 Deepfake；如果很小，可能是真实图片之间的自然差异。
    """
    def __init__(self, feature_dim):
        super().__init__()
        self.head = nn.Sequential(
            nn.Linear(feature_dim, 512),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(512, 1),
            nn.Sigmoid() # 输出 0-1 之间的异常分数
        )
        
    def forward(self, difference_feat):
        # 输入差异向量，输出异常分数
        return self.head(difference_feat)

# ==========================================
# 5. 完整模型主类 (Main Model)
# ==========================================
class DeepfakeContrastiveResidual(nn.Module):
    """
    Deepfake Contrastive Residual Network (DCR-Net)
    
    支持两种模式：
    1. mode='pair': 训练时使用。输入 (Img, Img_Pair)，显式计算差异。
    2. mode='single': 推理时使用。输入 (Img)，从 Memory Bank 检索基准计算差异。
    """
    def __init__(self, feature_dim=2048, memory_size=2048):
        super().__init__()
        
        # 1. 骨干网络
        self.backbone = FeatureExtractor(backbone_name='resnet50')
        
        # 2. 记忆库
        self.memory_bank = MemoryBank(feature_dim, bank_size=memory_size)
        
        # 3. 融合模块
        self.fusion = DifferenceAttention(feature_dim)
        
        # 4. 两个检测头
        # 主分类头 (Real vs Fake)
        self.classifier = nn.Linear(feature_dim, 2)
        # 辅助残差头 (Anomaly Score)
        self.residual_head = ResidualHead(feature_dim)
        
    def forward(self, x, x_pair=None, mode='single'):
        """
        前向传播逻辑
        
        Args:
            x: 主输入图像 [B, 3, 224, 224]
            x_pair: 配对图像 (仅 mode='pair' 时需要)
            mode: 'pair' | 'single' | 'update_memory'
        
        Returns:
            logits: 分类分数 [B, 2]
            anomaly_score: 残差异常分数 [B, 1]
            features: 最终用于分类的特征 [B, Dim] (用于可视化)
        """
        # 1. 提取主图像特征
        feat = self.backbone(x) # [B, 2048]
        
        difference_feat = None
        
        # ================== 模式分支 ==================
        if mode == 'update_memory':
            # 特殊模式：仅用于更新记忆库 (输入必须是真实图像)
            self.memory_bank.update(feat)
            return None
            
        elif mode == 'pair' and x_pair is not None:
            # === 训练模式 (Contrastive) ===
            # 提取配对图像特征
            feat_pair = self.backbone(x_pair)
            
            # 计算真实差异 (Feature Space Diff)
            # 使用 abs 差分保留幅度信息
            difference_feat = torch.abs(feat - feat_pair)
            
        else: # mode == 'single' or x_pair is None
            # === 推理/单样本模式 (Inference) ===
            if self.training:
                # 如果在训练时意外进入单模式，警告或使用自身作为基准(不推荐)
                pass 
            
            # 1. 从记忆库检索最近的“真实原型”
            # retrieved_feat 相当于“推测的该图真实样子”
            retrieved_feat = self.memory_bank.retrieve(feat)
            
            # 2. 计算虚拟差异 (Virtual Residual)
            difference_feat = torch.abs(feat - retrieved_feat)
        
        # ================== 融合与输出 ==================
        
        # 1. 辅助任务：通过残差头判断异常程度
        # 理论上：篡改图的 difference_feat 应该很大，真实图应该很小
        anomaly_score = self.residual_head(difference_feat)
        
        # 2. 交叉注意力融合
        # 让模型根据差异图，去加强原始特征中的伪造痕迹
        enhanced_feat = self.fusion(feat, difference_feat)
        
        # 3. 最终分类
        logits = self.classifier(enhanced_feat)
        
        return logits, anomaly_score, enhanced_feat

# ==========================================
# 6. 使用示例 (Example Usage)
# ==========================================
if __name__ == "__main__":
    # 实例化模型
    model = DeepfakeContrastiveResidual(feature_dim=2048)
    
    # 模拟输入数据
    batch_size = 4
    real_imgs = torch.randn(batch_size, 3, 224, 224) # 真实图
    fake_imgs = torch.randn(batch_size, 3, 224, 224) # 篡改图
    
    print("--- 1. 记忆库预热 (Warmup) ---")
    # 假设我们有一些真实图片用于构建基准
    model.eval()
    model(real_imgs, mode='update_memory')
    print("Memory bank updated.")
    
    print("\n--- 2. 训练阶段 (Pair Mode) ---")
    model.train()
    # 输入：真实图 和 对应的伪造图
    logits, ano_score, _ = model(fake_imgs, x_pair=real_imgs, mode='pair')
    print(f"Logits shape: {logits.shape}")          # [4, 2]
    print(f"Anomaly Score shape: {ano_score.shape}") # [4, 1]
    
    print("\n--- 3. 推理阶段 (Single Mode) ---")
    model.eval()
    # 输入：未知图像 (模拟伪造图)
    # 模型会自动去记忆库找最像的真实特征进行对比
    logits_test, ano_score_test, _ = model(fake_imgs, mode='single')
    print(f"Inference Logits: {logits_test[0].detach().cpu().numpy()}")
    print(f"Inference Anomaly Score: {ano_score_test[0].item():.4f}")
```

---

## 💡 关键实现细节注释

1. **`MemoryBank` 的设计**：
    
    - **Buffer 机制**：使用了 `register_buffer`。这意味着 `memory` 矩阵是模型状态的一部分（保存 PTH 文件时会带上），但在反向传播时**不会计算梯度**。
        
    - **更新策略**：代码中使用简单的 FIFO（先进先出）或循环覆盖。在实际大规模训练中，通常会在每个 Epoch 结束时，用整个训练集的真实样本特征重新构建一次记忆库。
    - 我觉得上面说的是一坨屎，所以看下面这个：[[memory bank 实现方案]]
        
2. **`DifferenceAttention` 的逻辑**：
    
    - 这里使用了 `Query` (原图特征) 和 `Key/Value` (差异特征)。
        
    - **物理含义**：模型在问：“基于我当前的特征（Query），差异部分（Key）中最显著的信息是什么？”然后将这些显著的差异信息（Value）加回到原特征中。这能帮助分类器聚焦于“因篡改而产生差异”的通道。
        
3. **`mode` 的切换**：
    
    - 训练时，你有 Ground Truth 的配对数据，所以用 `mode='pair'` 强行教会模型“什么是差异”。
        
    - 推理时，没有配对数据，模型通过 `mode='single'` 依赖记忆库来“回忆”正常的特征长什么样，从而模拟出差异。
        

## 📝 Next Step

你可以将此代码保存为 model.py。

下一步建议：编写一个自定义的 LossFunction，需要组合：

1. `CrossEntropyLoss` (针对 logits)
    
2. `BCELoss` (针对 anomaly_score，真实图pair标签为0，伪造图pair标签为1)
    
3. `ContrastiveLoss` (针对特征距离)

